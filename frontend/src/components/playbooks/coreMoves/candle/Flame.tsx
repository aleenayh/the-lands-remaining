...
export function Flame({ color }: { color: string }) {
	const colorObject = colors[color];
	const primaryColor = `hsl(${colorObject.h}, ${colorObject.s}%, ${colorObject.l}%)`;
	const secondaryColor = `hsl(${colorObject.h - 5}, ${colorObject.s - 10}%, ${colorObject.l - 10}%)`;
	const tertiaryColor = `hsl(${colorObject.h + 5}, ${colorObject.s + 10}%, ${colorObject.l + 10}%)`;
	const offsetDelay = useMemo(() => Math.random() * 1000, []);

	return (
		<div
			className="mo-fire w-full h-full"
			style={{ "--offset-delay": `${offsetDelay}ms` } as React.CSSProperties}
		>
			<svg
				version="1.1"
				id={`flame_${Math.random().toString(36).substring(2, 15)}`}
				xmlns="http://www.w3.org/2000/svg"
				xmlnsXlink="http://www.w3.org/1999/xlink"
				x="0px"
				y="0px"
				viewBox="45 0 90 189.864"
